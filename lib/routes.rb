...
helpers do
    ...
    def create_thumbnail(dragonfly_content)
        if valid_image_extensions.index(dragonfly_content
         .image_properties['format'])
            ratio = dragonfly_content.height / dragonfly_content.width
                    .to_f
    ...
    def meets_image_requirements?(dragonfly_content)
        (dragonfly_content.width <= 2048 &&
          dragonfly_content.height <= 2048 &&
            valid_image_extensions.index(dragonfly_content
             .image_properties['format']))
    end
    ...
    def upload_file_to_s3(dragonfly_content)
        if meets_image_requirements?(dragonfly_content)
            ...
            "(.#{dragonfly_content.image_properties['format']} or image is \
            too large" \
            "(#{dragonfly_content.width}x#{dragonfly_content.height})"
        end
    end
    
    #FIXME: change method parameters after adding functionality for
    #       logging in
    ...
    def upload_file_to_site(dragonfly_content)
        if valid_image_extensions.index(dragonfly_content
         .image_properties['format'])
            ...
            thumb_url
        elsif valid_nonimage_extensions
              .index(dragonfly_content.image_properties['format'])
    ...
    def legal_page_number?(n)
        ...
        if manager.get_post_details.cmd_tuples <= 20
            max_pages = 1
        elsif manager.get_post_details.cmd_tuples % 20 == 0
            max_pages = manager.get_post_details.cmd_tuples / 20
        else
            max_pages = (manager.get_post_details.cmd_tuples / 20.to_f)
                        .ceil
        end
        n <= max_pages
    end
