...
class DatabaseManager
   ...
   def initialize
      ...
      @region_ids = conn.exec('SELECT * FROM region').column_values(0)
                    .map(&:to_i)
   end
   ...
   def insert_images(img_hash, user_primary_key, region = nil)
      if img_hash[:original] && img_hash[:thumb]
         ...
         "(\'#{original_image_url.slice(original_image_url
         .index('//') + 2..original_image_url.length - 1)}\'," \
         "\'#{thumbnail_image_url.slice(thumbnail_image_url
         .index('//') + 2..thumbnail_image_url.length - 1)}\')")

         case picture_table_insert_result
         when PG::Result
            last_picture_row = conn.exec('SELECT picture_id FROM ' \
            'picture WHERE' + " original_image_url=\'#{original_image_url
            .slice(original_image_url.index('//') + 2..original_image_url
            .length - 1)}\'")
            recently_inserted_row_primary_key = last_picture_row[0]['picture_id']
            insert_post_details(
               recently_inserted_row_primary_key, user_primary_key, region
            )
         else
   ...
   def insert_comment(message, picture_primary_key, user_primary_key)
      conn.exec('INSERT INTO comment(message, ' \
   ...
   def get_gallery_pertinent_records(page)
      ...
      if offset > 0
         conn.exec('SELECT * FROM (SELECT * FROM post_details' \
